...
export default async function SolicitacoesPage() {
  ...
  const [{ data: unidades }, { data: convenios }, { data: solicitacoes }] = await Promise.all([
    ...
    supabase
      .from("solicitacoes")
      .select(
        "id, numero_requisicao, nome_paciente, acao, status, data_solicitacao, exame_texto_livre, recepcionista_nome, exames(nome), unidades(nome), convenios(nome), perfis!solicitacoes_solicitante_id_fkey(nome)"
      )
